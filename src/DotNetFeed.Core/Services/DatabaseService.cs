...
using DotNetFeed.Events;
using DotNetFeed.Exceptions;
...
public class DatabaseService : IDisposable
{
    ...
    public async Task<FeedFolder?> GetFeedFolderAsync(int id)
    {
        if (!this.isInitialized)
        {
            return null;
        ...
        try
        {
            return await this.database.Table<FeedFolder>().FirstOrDefaultAsync(n => n.Id == id);
        }
        catch (Exception ex)
        {
            this.errorHandler.HandleError(new DatabaseException("Error getting FeedFolder.", ex));
            return null;
        }
    }

    public async Task<FeedListItem[]> GetFeedListItemsForFolderAsync(int folderId)
    {
        if (!this.isInitialized)
        {
            return Array.Empty<FeedListItem>();
        ...
        try
        {
            return await this.database.Table<FeedListItem>().Where(n => n.FolderId == folderId).ToArrayAsync();
        }
        catch (Exception ex)
        {
            this.errorHandler.HandleError(new DatabaseException("Error getting FeedListItems.", ex));
    ...
    public async Task<FeedFolder[]> GetFeedFoldersAsync()
    {
        if (!this.isInitialized)
        {
            return Array.Empty<FeedFolder>();
        }

        try
        {
            return await this.database.Table<FeedFolder>().ToArrayAsync();
        }
        catch (Exception ex)
        {
            this.errorHandler.HandleError(new DatabaseException("Error getting FeedFolders.", ex));
            return Array.Empty<FeedFolder>();
        }
    }
    ...
    /// Upsert a FeedFolder.
    /// </summary>
    /// <param name="folder">FeedFolder.</param>
    /// <returns>Number of rows changed.</returns>
    public async Task<int> UpsertFolderAsync(FeedFolder folder)
    {
        if (!this.isInitialized)
        {
            return 0;
        ...
        try
        {
            ...
            if (folder.Id <= 0)
            {
                result = await this.database.InsertAsync(folder, typeof(FeedFolder));
            }
            else
            {
                result = await this.database.UpdateAsync(folder, typeof(FeedFolder));
            }
        ...
        {
            this.errorHandler.HandleError(new DatabaseException("Error upserting FeedFolder.", ex));
        }
